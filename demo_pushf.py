...
def main(output, render_size, control_hz):
    """
    Collect demonstration for the Push-F task.

    Usage: python demo_pushf.py -o data/pushf/pushf_demo.zarr

    This script is compatible with both Linux and MacOS.
    Hover mouse close to the blue circle to start.
    Push the F block into the green area.
    ...
    env = PushTKeypointsEnv(render_size=render_size, render_action=False, block_shape='f', **kp_kwargs)
    ...
    while True:
        ...
        plan_idx = 0
        pygame.display.set_caption(f'Push-F Demo | Episode {seed}')
